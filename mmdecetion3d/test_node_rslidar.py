...
def process_point_cloud(data):
    ...
    pc = np.frombuffer(data.data, dtype=dtype_list)
    ...
    for i, (x, y, z, dx, dy, dz, angle, cls, score ) in enumerate(output_bboxes):
        ...
        marker_boxes = Marker()
        marker_boxes.header.frame_id = "rslidar"
        ...
        marker_array.markers.append(marker_boxes)
            # 3、做这个检测的文本标签
        marker_word = Marker()
        marker_word.header.frame_id = "rslidar"
