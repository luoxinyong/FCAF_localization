...
double filter_size_surf_min = 0.5, filter_size_map_min = 0.5, filter_size_save_min = 0.5, filter_size_show_min = 0.5, fov_deg = 180;
...
bool ifLoop = false, ifPCL = true;
double mappingProcessInterval;
double kdtreeResetDistance, kdtreeResetTime;
double loopHeightMax, loopHeightMin;
int currentKeyframeSearchNum;
double historyKeyframeSearchRadius, historyKeyframeFitnessScore, historyKeyframeSearchTimeDiff, surroundingkeyframeAddingDistThreshold, surroundingkeyframeAddingAngleThreshold;
int historyKeyframeSearchNum;

bool ifFloorDetect = false;
double detect_rate;

int scan_save_interval;
bool detect_object_en;

void readParameters(ros::NodeHandle &nh)
{
  p_pre.reset(new Preprocess());
  p_imu.reset(new ImuProcess());
  nh.param<double>("loopClosure/historyKeyframeSearchRadius",historyKeyframeSearchRadius,10.0);
  nh.param<double>("loopClosure/historyKeyframeFitnessScore",historyKeyframeFitnessScore,0.3);
  nh.param<double>("loopClosure/historyKeyframeSearchTimeDiff",historyKeyframeSearchTimeDiff,10.0); 
  nh.param<int>("loopClosure/historyKeyframeSearchNum",historyKeyframeSearchNum,10.0); 
  nh.param<double>("loopClosure/surroundingkeyframeAddingDistThreshold",surroundingkeyframeAddingDistThreshold,1.0);
  nh.param<double>("loopClosure/surroundingkeyframeAddingAngleThreshold",surroundingkeyframeAddingAngleThreshold,0.2);
  nh.param<double>("loopClosure/mappingProcessInterval",mappingProcessInterval,0.2);
  nh.param<double>("loopClosure/kdtreeResetDistance",kdtreeResetDistance,0.2);
  nh.param<double>("loopClosure/kdtreeResetTime",kdtreeResetTime,0.2);
  nh.param<double>("loopClosure/loopHeightMax",loopHeightMax,1);
  nh.param<double>("loopClosure/loopHeightMin",loopHeightMin,-1);
  nh.param<bool>("loopClosure/ifPCL",ifPCL,true);
  nh.param<int>("loopClosure/currentKeyframeSearchNum",currentKeyframeSearchNum,5);
  nh.param<bool>("loopClosure/ifLoop",ifLoop,false);
  ...
  nh.param<double>("filter_size_save",filter_size_save_min,0.5);
  nh.param<double>("filter_size_show",filter_size_show_min,0.5);
  ...
  nh.param<int>("detectObject/save_interval", scan_save_interval, -1);
  nh.param<bool>("detectObject/detect_object_en", detect_object_en, false);
  nh.param<bool>("detectObject/ifFloorDetect",ifFloorDetect,false);
  nh.param<double>("detectObject/floor_detect_rate",detect_rate,0.5);
